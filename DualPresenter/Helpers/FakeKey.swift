...
class FakeKey {
	...
	private static let keyCodeRange = 0x00...0x7E
	
	class func send(fakeKey key: String, useCommandFlag: Bool) {
		let keyCode = stringToKeyCode(key)
		send(fakeKey: numericCast(keyCode), useCommandFlag: useCommandFlag)
	}
	
	private class func transformKeyCode(_ keyCode: Int) -> String {
	...
	private class func keyCodeTranslationTable() -> [String: Int] {
	...
	private class func stringToKeyCode(_ key: String) -> Int {
	...
	private class func send(fakeKey keyCode: CGKeyCode, useCommandFlag: Bool) {
